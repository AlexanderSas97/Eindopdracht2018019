...
void MainWindow::on_input_clicked()
{
    ...
    cvtColor(myProcessedIm.inputMat, input, cv::COLOR_GRAY2BGR);
    myProcessedIm.inputMade = true;
    //}
...
//RUN AND TIME CV:
    std::cout << "starting CV" << std::endl;
    std::chrono::time_point<std::chrono::steady_clock> startCV = std::chrono::steady_clock::now();  //record starttime
    myProcessedIm.CV_Algorithm();                                                                   //Calculate CV
    std::chrono::time_point<std::chrono::steady_clock> stopCV = std::chrono::steady_clock::now();   //record stoptime
    std::cout << "CV DONE" << std::endl;

//OUTPUT CV-RESULTS ON UI:
    QString strCV1;                                                                                 //Initialize QString with results

    //Report amount of ellipses:
    strCV1.QString::setNum(myProcessedIm.CVRecognizedEllipses.size());
    strCV1.append(" ellipses found by the CV algorithm \n \n");

    //Report duration:
    long long timeCV = std::chrono::duration_cast<std::chrono::milliseconds>(stopCV - startCV).count();     //Calculate duration
    QString timeTakenCV = "CV-duration (ms): \n " + QString::fromStdString(std::to_string(timeCV)) + " \n"; //Store duration in QString
    strCV1.append(timeTakenCV);                                                                               //Display duration in output-QString

    //Report data of found CV-ellipses:
...
//RUN AND TIME XIE:
    ...
    std::chrono::time_point<std::chrono::steady_clock> startXIE = std::chrono::steady_clock::now();
    myProcessedIm.Xie_Algorithm(ui->minVal2aSpinBox->value(), ui->minVal2bSpinBox->value(), ui->tresholdSpinBox->value());
    std::chrono::time_point<std::chrono::steady_clock> stopXIE = std::chrono::steady_clock::now();
    std::cout << "Xie DONE" << std::endl;

//OUTPUT XIE-RESULTS ON UI:
    QString strXie1;                                                                                //Initialize QString with results
    ...
    //Report duration:
    long long timeXIE = std::chrono::duration_cast<std::chrono::milliseconds>(stopXIE - startXIE).count();     //Calculate duration
    QString timeTakenXIE = "XIE-duration (ms): \n " + QString::fromStdString(std::to_string(timeXIE)) + "\n"; //Store duration in QString
    strXie1.append(timeTakenXIE);                                                                               //Display duration in output-QString

    //Report Data of found XIE-ellipses:
